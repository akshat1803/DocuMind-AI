...
export default function ChatWorkspace() {
  ...
  const conversation = useQuery({ queryKey: ['conversation', conversationId], queryFn: () => conversationsService.get(conversationId), enabled: Boolean(conversationId) && !isDraft });
  const draftDocuments = useQuery({ queryKey: ['documents'], queryFn: documentsService.list, enabled: isDraft });
  ...
  const messages = useMemo(() => isDraft ? [] : conversation.data?.conversation.messages ?? [], [conversation.data, isDraft]);
  const selectedDocuments = useMemo(() => {
    if (!isDraft) return conversation.data?.conversation.documents ?? [];
    const requested = new Set(draftDocumentIds);
    return (draftDocuments.data?.documents ?? [])
  ...
  }, [conversation.data, draftDocumentIds, draftDocuments.data, isDraft]);
  ...
  if ((isDraft && draftDocuments.isLoading) || (!isDraft && conversation.isLoading)) return <div className="grid min-h-screen place-items-center text-sm text-slate-500">Loading conversation…</div>;
  if (isDraft && (draftDocumentIds.length === 0 || selectedDocuments.length !== draftDocumentIds.length)) return <div className="grid min-h-screen place-items-center text-sm text-red-600"><div className="text-center"><p>The selected PDFs are unavailable.</p><Link to="/" className="mt-3 inline-block font-semibold text-sky-700">Return to documents</Link></div></div>;
  if (!isDraft && !conversation.data) return <div className="grid min-h-screen place-items-center text-sm text-red-600">Conversation could not be loaded.</div>;
  const selectionUrl = `/?documents=${encodeURIComponent([...documentIds].sort().join(','))}`;
  const title = isDraft ? 'New conversation' : conversation.data!.conversation.title;
